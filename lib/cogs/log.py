...
class Log(Cog):
    ...
    async def on_message_edit(self, before:Message, after:Message):
        if not after.author.bot:
            ...
            db_sqlite.execute("INSERT OR REPLACE INTO messages (MessageID, guild, channel, author, time, message, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    async def on_message_delete(self, message: Message):
        if not message.author.bot:
            ...
            db_sqlite.execute("INSERT OR REPLACE INTO messages (MessageID, guild, channel, authot, time, message, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
